...
public class UI : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        if (GUI.Button(new Rect(Screen.width / 2 -250, 0, 120, 40), "重新开始"))
        {
            action.ReStart();
        }
        if(sceneController.gameState == GameState.END)
        {
            GUI.Label(new Rect(Screen.width / 2-100, Screen.height/2-100, 200, 80),"游戏结束!", fontstyle1);
        }
        GUI.Label(new Rect(Screen.width / 2, 0, 50, 20), "当前分数: " + sceneController.scoreRecorder.GetScore().ToString(), fontstyle1);
    }
